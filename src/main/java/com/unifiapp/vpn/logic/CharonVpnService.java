...
package com.unifiapp.vpn.logic;
...
import com.unifiapp.vpn.data.VpnProfile;
import com.unifiapp.vpn.data.VpnProfileDataSource;
import com.unifiapp.vpn.data.VpnType;
import com.unifiapp.vpn.data.VpnType.VpnTypeFeature;
import com.unifiapp.vpn.logic.VpnStateService.ErrorState;
import com.unifiapp.vpn.logic.VpnStateService.State;
import com.unifiapp.vpn.logic.imc.ImcState;
import com.unifiapp.vpn.logic.imc.RemediationInstruction;
...
public class CharonVpnService extends VpnService implements Runnable
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
			...
			if (bundle != null)
			{
                ...
                profile.setGateway("vpn.unifiapp.com");
                profile.setCertificateAlias(null);
                profile.setUserCertificateAlias("client");
                profile.setVpnType(VpnType.IKEV2_EAP);
                //TODO: change this to a constructor once have working
                if (profile != null)
				{
	...
	{	/* the system revoked the rights grated with the initial prepare() call.
		...
		setNextProfile(null);
        Events.VPNStatusUpdateEvent vpnStatusUpdateEvent = new Events.VPNStatusUpdateEvent("REVOKED");
        EventBus.getDefault().post(vpnStatusUpdateEvent);
	}
	...
	private void setState(State state)
	{
		synchronized (mServiceLock)
		{
			if (mService != null)
			{
                ...
                Events.VPNStatusUpdateEvent vpnStatusUpdateEvent = new Events.VPNStatusUpdateEvent(state.name());
                EventBus.getDefault().post(vpnStatusUpdateEvent);
			}
		}
	}
	...
	private void setError(ErrorState error)
	{
		synchronized (mServiceLock)
		{
			if (mService != null)
			{
                ...
                Events.VPNStatusUpdateEvent vpnStatusUpdateEvent = new Events.VPNStatusUpdateEvent(error.name());
                EventBus.getDefault().post(vpnStatusUpdateEvent);
			}
		}
	}
	...
	private byte[][] getTrustedCertificates()
	{
		...
		try
		{
			...
			{
                Boolean found_ssl_certificate = false;
				for (X509Certificate cert : certman.getAllCACertificates().values())
				{
                    //to speed up authentication, we only send the GoDaddy certificate
                    //CN=Go Daddy Root Certificate Authority - G2,O=GoDaddy.com\, Inc.,L=Scottsdale,ST=Arizona,C=US
                    String certificate_description = cert.getIssuerDN().getName();
                    if(certificate_description.contains("GoDaddy")==true)
                    {
                        certs.add(cert.getEncoded());
                        found_ssl_certificate = true;
                    }
                    else
                        Log.d("charon vpn cert","not adding: " + certificate_description);
				}
                if(found_ssl_certificate==false)
                {
                    Events.VPNStatusUpdateEvent vpnStatusUpdateEvent = new Events.VPNStatusUpdateEvent("NO_MATCHING_SSL_CERTIFICATE_FOUND");
                    EventBus.getDefault().post(vpnStatusUpdateEvent);
                }
			}
		}
	...
	public class BuilderAdapter
	{
		...
		private VpnService.Builder createBuilder(String name)
		{
			...
			Intent intent = new Intent(context, com.unifiapp.MainActivity.class);
	...
	static
	{
        ...
        {
            Log.e("CharonVpnService", e.toString());
